    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if self.fold < len(splits):
                ...
                # print(keys)
                keys_new = [key for key in keys if self.prefix in key]
                if len(keys_new)>0:
                    keys=keys_new
                # print(keys)
                self.dataset_len=len(keys)
        ...
        print(tr_keys,val_keys)
        '''
        for key in tr_keys:
            #print (key)
            if self.prefix == None:
                self.dataset_tr[key] = self.dataset[key]
            elif (key.startswith(self.prefix)):
                self.dataset_tr[key] = self.dataset[key] #startswith
                #print(key)
        
        for key in val_keys:
            if self.prefix == None:
                self.dataset_val[key] = self.dataset[key]
            elif (key.startswith(self.prefix)):
                self.dataset_val[key] = self.dataset[key] #startswith
        '''
